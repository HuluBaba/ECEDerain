...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, aggregate=True):
        ...
        self.ffn = DecoupleConvv2(dim, dim, dim*3, aggregate=aggregate)
    ...
    def __init__(self,
        ...
        self.latent = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 3), num_heads=heads[3], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type, aggregate=False) for i in range(num_blocks[3])])
