...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type):
        ...
        self.ffn = DeCoupleConvv5(dim, ffn_expansion_factor, bias)
...
if __name__=="__main__":
    ...
    model=TransformerBlock(dim*8, num_heads=8, ffn_expansion_factor=2.66, bias=False,
                             LayerNorm_type='WithBias')
    getModelSize(model)
    model=DeCoupleConvv5(dim)
    getModelSize(model)
    model=DeCoupleConvv5(dim*4)
    getModelSize(model)
    model=DeCoupleConvv5(dim*8)
    getModelSize(model)
