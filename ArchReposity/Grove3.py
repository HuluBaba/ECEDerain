...
class IDI(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x * se_k.unsqueeze(-1).unsqueeze(-1)
        return x
...
class Stage(nn.Module):
    def __init__(self,dim,num_heads, ffn_expansion_factor,bias,LayerNorm_type,num_blocks,lp_guide=True,sb_guide=False):
...
class Grove3(nn.Module):
    ...
    def forward(self, input):
        ...
        f_e1_o = self.fencoder_level1(f_e1_i)
        p_e1_o = self.encoder_level1(p_e1_i, f_e1_o)

        p_e2_i = self.down1_2(p_e1_o)
        f_e2_i = self.fdown1_2(f_e1_o)

        f_e2_o = self.fencoder_level2(f_e2_i)
        p_e2_o = self.encoder_level2(p_e2_i, f_e2_o)

        p_e3_i = self.down2_3(p_e2_o)
        f_e3_i = self.fdown2_3(f_e2_o)

        f_e3_o = self.fencoder_level3(f_e3_i)
        p_e3_o = self.encoder_level3(p_e3_i, f_e3_o)

        p_latent_i = self.down3_4(p_e3_o)
        f_latent_i = self.fdown3_4(f_e3_o)

        f_latent_o = self.flatent(f_latent_i)
        p_latent_o = self.latent(p_latent_i, f_latent_o)
        ...
        f_d3_o = self.fdecoder_level3(f_d3_i)
        p_d3_o = self.decoder_level3(p_d3_i, f_d3_o)
        ...
        f_d2_o = self.fdecoder_level2(f_d2_i)
        p_d2_o = self.decoder_level2(p_d2_i, f_d2_o)

        p_d1_i = self.up2_1(p_d2_o)
        p_d1_i = torch.cat([p_d1_i, p_e1_o], 1)
        f_d1_i = self.fup2_1(f_d2_o)

        f_d1_o = self.fdecoder_level1(f_d1_i)
        p_d1_o = self.decoder_level1(p_d1_i, f_d1_o)
